...
q2_df = tibble(
  ...
  x_plus_y_factor = factor(x_plus_y_numeric)
)
```


The size of the data set is `r nrow(q2_df)` rows and `r ncol(q2_df)` columns. The mean of x is `r mean(q2_df$x)` and the median of x is `r median(q2_df$x)`. The proportion of cases for which the logical vector is TRUE is `r mean(q2_df$x_plus_y_logical)`.
